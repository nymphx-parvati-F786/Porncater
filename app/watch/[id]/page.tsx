...
import { Heart, Share2, Download, Sparkles, Film, Eye, User } from 'lucide-react';
    ...
    const loadData = async () => {
      try {
        ...
        if (videoRes.ok) {
          ...
          const safeVideos = Array.isArray(allVideosData) ? allVideosData : (allVideosData.data || []);
          const filteredVideos = safeVideos.filter((v: Video) => v.id !== parseInt(id));
          ...
          const sortedPornstars = safePornstars.sort((a: any, b: any) => (b.views || 0) - (a.views || 0)).slice(0, 5);
  ...
  if (!video) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center text-zinc-400 font-light tracking-widest uppercase">
        ...
        <button onClick={() => router.push('/')} className="text-rose-800 hover:text-rose-600 transition underline underline-offset-4">Return Home</button>
      </div>
    );
  }

  // Format the date
  const uploadDate = video.createdAt ? new Date(video.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'Recently Uploaded';
      ...
      <div className="max-w-[1400px] mx-auto px-6 pt-10">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {video.pornstars.map((star) => (
                           <Link key={star.id} href={`/pornstars/${star.id}`} className="flex items-center gap-2 bg-zinc-900/50 hover:bg-zinc-800 border border-zinc-800 rounded-full pr-3 py-0.5 transition-colors">
        ...
        <div className="mt-16 border-t border-white/5 pt-16 pb-16">
          <div className="flex items-center gap-3 mb-10">
            ...
            <h3 className="text-2xl font-serif italic text-white tracking-wide">More Porn Videos</h3>
          </div>
              ...
              <Link key={v.id} href={`/watch/${v.id}`} className="group block cursor-pointer">
                ...
                <div className="mt-3 px-1">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-zinc-500 text-[10px] uppercase tracking-widest">
                    <span className="flex items-center gap-1"><Eye size={12}/> {Number(v.views || 0).toLocaleString()}</span>
                  </div>
                </div>
              </Link>
            ))}
