...
export default function InboxClientLayout({ initialMessages }: { initialMessages: SerializedMessage[] }) {
    ...
    // 🎬 THE GMAIL SECRET SAUCE: Injects modern typography and responsive media rules
    const renderCinematicHtml = (rawHtml: string) => {
        const defaultStyles = `
      <style>
        /* Force modern corporate typography */
        body {
        ...
        /* Make all banners, images, and embedded videos fully responsive */
        img, video, iframe {
        ...
        /* Force ancient email tables to fit the screen */
        table { 
          max-width: 100% !important; 
    ...
    `;
        ...
        // If the email brings its own <head>, inject our styles safely inside it
        if (rawHtml.toLowerCase().includes('<head>')) {
            return rawHtml.replace(/<head>/i, `<head>\n${defaultStyles}`);
        }

        // Otherwise, wrap their raw text in a flawless modern HTML5 shell
        return `<!DOCTYPE html><html><head>${defaultStyles}</head><body>${rawHtml}</body></html>`;
    };
