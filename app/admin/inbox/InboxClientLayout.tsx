...
export default function InboxClientLayout({ initialMessages }: { initialMessages: SerializedMessage[] }) {
  // We use client state to instantly update the UI when you click delete/read
  const [messages, setMessages] = useState<SerializedMessage[]>(initialMessages);
  const [activeId, setActiveId] = useState<number | null>(null);
  const [filter, setFilter] = useState<string>("all");
  const [searchQuery, setSearchQuery] = useState<string>("");

  // Helper to extract clean names from strings like: "John Doe <john@gmail.com>"
  ...
  // Filter engine (handles Search + Folders + Trash)
  ...
  // Action: Move to Trash / Restore
  const handleToggleTrash = async (id: number, e: React.MouseEvent) => {
    e.stopPropagation(); // Stop the row click from opening the email
    const msg = messages.find(m => m.id === id);
    if (!msg) return;

    const newTrashState = !msg.isTrashed;
    
    // Optimistic UI update instantly removes it from current view
    setMessages(prev => prev.map(m => m.id === id ? { ...m, isTrashed: newTrashState } : m));
    if (activeId === id) setActiveId(null); // Close the viewer if deleted
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-200 font-sans flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden h-[calc(100vh-64px)]">
        ...
        {/* READING PANE */}
        <main className="flex-1 bg-[#030303] flex flex-col h-full overflow-hidden">
          {activeMessage ? (
            <div className="flex flex-col h-full">
              {/* Email Header */}
              <div className="p-8 pb-6 border-b border-white/5 shrink-0">
                <h2 className="text-2xl text-white font-light mb-6">{activeMessage.subject}</h2>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <UserCircle2 size={36} strokeWidth={1} className="text-zinc-600" />
                    <div>
                      <div className="text-sm font-medium text-zinc-200">{parseSender(activeMessage.from).name}</div>
                      <div className="text-xs text-zinc-500 font-mono mt-0.5">&lt;{parseSender(activeMessage.from).email}&gt;</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 text-zinc-500 text-xs bg-black/40 px-3 py-1.5 rounded-sm border border-white/5">
                    <Clock size={12} />
              ...
              {/* Email Body Sandbox */}
              <div className="flex-1 bg-white overflow-hidden relative m-4 md:m-8 rounded-md shadow-2xl">
                {activeMessage.isHtml ? (
                  ...
                  <div className="w-full h-full p-8 text-sm text-zinc-800 whitespace-pre-wrap overflow-y-auto bg-white font-sans">
                    {activeMessage.body}
                  </div>
                )}
              </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-zinc-600">
              ...
              <p className="text-sm font-light">Select an item to read</p>
            </div>
          )}
        </main>
      </div>
