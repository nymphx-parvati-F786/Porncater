...
export async function GET(
  ...
  const skipAmount = chunkId * CHUNK_SIZE;

  const videos = await prisma.video.findMany({
    where: { status: "PUBLISHED" },
    select: {
      id: true, slug: true, title: true, thumbnail: true,
      duration: true, views: true, category: true, tags: true, createdAt: true,
  ...
  videos.forEach((video) => {
    const pageUrl = `${BASE_URL}/video/${video.id}/${video.slug}`;
    ...
    if (video.tags && video.tags.length > 0) video.tags.forEach((tag) => combinedTags.add(tag));
    if (video.pornstars && video.pornstars.length > 0) video.pornstars.forEach((star) => combinedTags.add(star.name));
