...
export default function PornstarProfile() {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        // Fetch pornstar data (adjust endpoint if needed)
        const starRes = await fetch(`/api/pornstars/${starId}`);
        if (starRes.ok) {
          const starData = await starRes.json();
          setStar(starData);
        }

        // Fetch videos linked to this pornstar ID
    ...
    <div className="min-h-screen bg-[#050505] text-zinc-200 font-sans selection:bg-rose-900 selection:text-white pb-24">
          ...
          <div className="flex-1 pb-2 w-full">
            ...
            <div className="flex flex-wrap items-center gap-8 mt-8 py-4 border-y border-white/5">
              <div className="flex flex-col">
                <span className="text-xl font-light text-white">{videos.length}</span>
                <span className="text-[9px] uppercase tracking-widest text-zinc-500">Porn Videos</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xl font-light text-white">{star.views ? Number(star.views).toLocaleString() : '0'}</span>
      ...
      <div className="max-w-[1400px] mx-auto px-6 mt-20">
        <div className="flex items-center gap-3 mb-10 border-b border-white/5 pb-4">
          ...
          <h3 className="text-2xl font-serif italic text-white tracking-wide">Porn Videos featuring {star.name}</h3>
        </div>
              ...
              <Link key={video.id} href={`/watch/${video.id}`} className="group block cursor-pointer">
                ...
                <div className="mt-4 px-1">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-zinc-500 text-[10px] uppercase tracking-widest">
                    <span className="flex items-center gap-1"><Eye size={12}/> {video.views ? Number(video.views).toLocaleString() : '0'}</span>
