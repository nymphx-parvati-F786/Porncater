...
// Define exactly what happened to the URL during processing
...
// ------------------------------------------------------------------
// MAIN PROCESSING ENGINE (Now with built-in Retries)
// ------------------------------------------------------------------
async function processVideoUrl(targetUrl: string, maxRetries = 3): Promise<ProcessResult> {
  console.log(`\n=========================================`);
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      // Return success type based on whether it took retries to get here
      ...
      if (attempt <= maxRetries) {
        console.log(`[Cooling] Pausing for 5 seconds before trying again...\n`);
...
// ------------------------------------------------------------------
// PIPELINE RUN ROUTINE (Now with Logger & Analytics)
// ------------------------------------------------------------------
async function run() {
  ...
  for (const url of urlsToScrape) {
    ...
    // 3 second cooling down offset window between completely different URLs
    await new Promise(resolve => setTimeout(resolve, 3000));
  }

  // --- FINAL PIPELINE LOGGER REPORT ---
  console.log(`\n=========================================`);
  ...
  if (failedUrls.length > 0) {
    // Write failed URLs to a new file so you can run them through a proxy or check manually
